...
//	}
	...
	private static List<List<Object>> read2007Excel(File file) throws Exception {
		...
		for (int i = sheet.getFirstRowNum(); i <= sheet.getPhysicalNumberOfRows(); i++) {
			...
			if (cell == null) {
				...
				linked.add(row.getCell(6).toString().trim());
			} else {
				continue;
			}
			list.add(linked);
	 ...
	 * 读取Office 2007 excel
	 */
	public static List<MappingExcel> read2007Excel4EvkAPPTestCase(int sheetNo, File file, String fileName) throws Exception {

		List<MappingExcel> list = new LinkedList<MappingExcel>();
		XSSFWorkbook xwb = new XSSFWorkbook(new FileInputStream(file+"\\"+fileName));
		...
		for (int i = sheet.getFirstRowNum(); i <= sheet.getPhysicalNumberOfRows(); i++) {
			...
			MappingExcel me = new MappingExcel();
				...
				if(row.getCell(0) != null && !row.getCell(0).equals(""))
					//linked.add(row.getCell(0).toString().replaceAll("[\\t\\n\\r]", ""));
					me.setName(row.getCell(0).toString().replaceAll("[\\t\\n\\r]", ""));
				if(row.getCell(1) != null && !row.getCell(1).equals(""))
					me.setValue(row.getCell(1).toString().replaceAll("[\\t\\n\\r]", ""));
				if(row.getCell(2) != null && !row.getCell(2).equals(""))
					me.setType(row.getCell(2).toString().replaceAll("[\\t\\n\\r]", ""));
				list.add(me);
	...
	public static String getInsertSqlByList(@org.jetbrains.annotations.NotNull List<List<Object>> alist, String fileName){
		String sql = "INSERT INTO xiaomi_testcase(testcase_id,testcase_name,testcase_title," +
				"testcase_priority,testcase_method,testcase_model,testcase_precondition,testcase_step," +
				"testcase_expected_result,testcase_actual_result_android,testcase_actual_result_ios," +
				"testcase_bug_id,testcase_remark,testcase_file_name) VALUES ";
		for(int i = 0 ; i < alist.size() ; i++){
			sql+="(";
			List blist = alist.get(i);
			for(int j = 0 ; j < blist.size() ; j++){
				String result = blist.get(j).toString();
				if(result.indexOf("'") > -1)
